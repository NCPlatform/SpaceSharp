...
const HotelDetailReview = ({ seqHotel }) => {
  ...
  const onSubmit = () => {
    if (commentDTO.seqReservation === 0) {
      ...
      formData.append(
        "commentDTO",
        new Blob([JSON.stringify(commentDTO)], { type: "application/json" })
      );
  ...
  return (
    <div>
          ...
          <div>
                      ...
                      <p className="fs-5">
                        <span className="fw-bold">
                          {reserveUser.filter(
                            (user) =>
                              user.email ===
                              review[reviewPage * 3 + index].email
                          )[0].name + " "}
                        </span>
                        고객님
                      </p>
                      <p>
                        {Array.from({ length: "5" }).map((_, index2) => (
                          <ImStarFull
                            key={index2}
                            className={
                              index2 <= review[reviewPage * 3 + index].rating
                                ? "starLatingGray"
                                : "starLatingBlack"
                            }
                            size="25"
            ...
            <div className="text-center mt-3">
              <div className="btn-group" role="group">
                ...
                {Array.from({ length: (review.length - 1) / 3 + 1 }).map(
                  (_, pIndex) => {
                    return (
                      <button
                        type="button"
                        className={
                          reviewPage === pIndex
                            ? "btn px-3 fw-bold text-dark"
                            : "btn px-3 text-secondary"
                        }
                        key={pIndex}
                ...
                <button
                  ...
                  disabled={
                    reviewPage === (review.length - 1) / 3 ? true : false
                  }
                >
                  {">"}
                </button>
      ...
      <div className="bg-dark bg-opacity-10 p-3 rounded">
        {sessionUserDTO ? (
          reservation ? (
            reservation.filter((item) => item.email === sessionUserDTO.email)
              .length > 0 ? (
              <div>
                <h3>리뷰작성</h3>
                <p>언제 예약에 대한 리뷰인가요</p>
                        ...
                        <div key={index} className="col">
                          <div
                            ...
                            onClick={() => {
                              review.filter(
                                (review) =>
                                  review.seqReservation === item.seqReservation
                              ).length > 0
                                ? Swal.fire({
                                    icon: "error",
                                    text: "이미 리뷰가 작성된 예약입니다",
                                  })
                                : commentDTO.seqReservation ===
                                  item.seqReservation
                                ? setCommentDTO({
                                    ...commentDTO,
                                    seqReservation: 0,
                            ...
                            <div className="card-body">
                              <p className="card-text">
                                ...
                                <p className="p-0 m-0 text-center">
                                  {moment(item.travelStartDate).format(
                                    "YYYY/MM/D hh:mm"
                                  ) + " "}
                                  ~
                                  {" " +
                                    moment(item.travelEndDate).format(
                                      "YYYY/MM/D hh:mm"
                                    )}
                                </p>
                              </p>
