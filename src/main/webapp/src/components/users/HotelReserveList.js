    ...
    <div style={{ minHeight: '100vh' }}>
      ...
      <div className="container" style={{ minHeight: '63.8vh' }}>
        ...
        <div className="text-end">
          <button className="btn" onClick={() => (setSort('travelStartDate'), setPage(0), setSemiPage(0))}>
            이용일자순정렬
          </button>
          /
          <button className="btn" onClick={() => (setSort('seqReservation'), setPage(0), setSemiPage(0))}>
            예약번호순정렬
          </button>
        </div>
        {reservationPage && hotelList && (
          <div className="row row-cols-1 row-cols-lg-2 row-cols-xl-2">
            {reservationPage.map((item, index) => (
              <HotelReservationItem key={index} item={item} hotel={hotelList[index]} />
            ))}
          </div>
        )}
