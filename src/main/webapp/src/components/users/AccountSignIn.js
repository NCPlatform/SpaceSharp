...
const SignIn = ({ userInfo }) => {
    ...
    naverLogin.getLoginStatus(async function (status) {
      if (status) {
        ...
        try {
              ...
              if (iskakao) {
                ...
                Swal.fire({
                  ...
                  if (result.isConfirmed) {
                    ...
                    // 통합을 하지 않는다고 하면 로그인에 실패했습니다. 라는 알람과 함께 로그인 페이지로 이동
                    Swal.fire({
                      title: '로그인에 실패하였습니다.',
                      imageUrl:
                ...
                // 네이버와 카카오 모두 연동이 되어있는 아이디가 아니라면 네이버에서 email, name, nickname 정보를 받아와서 로컬 스토리지에 저장
                try {
                  // post로 백엔드에 요청 (isnaver 값을 true로 바꾸는 것)
                  axios
                    .get(`/user/updateNaverStatus?userEmail=${userid}`)
                    .then(res => {
                      ...
                      Swal.fire({
                        title: '로그인 완료.',
                        imageUrl:
                          'https://item.kakaocdn.net/do/d640911d600b52c55d356740537ae9569f5287469802eca457586a25a096fd31',
                        imageWidth: 300,
                        imageHeight: 200,
                        imageAlt: '구데타마',
                      });
          ...
          // userid 라는 이름으로 email 정보 가져오는 요청 했던 try 문에 대한 catch
          console.error('에러 발생:', error);
          // 에러 처리 로직 추가
        }
      }
  ...
  const passwordRegex =
    /^(?=.*[A-Za-z])(?=.*\d|.*[!@#$%^&*()_+])(?!.*(abc|def|ghi|jkl|mno|pqr|stu|vwx|yz|123|234|345|456|567|678|789|890)).{8,30}$/;
  ...
  const onFocusEmail = () => {
    if (userDTO.email) {
      ...
      if (userDTO.email) {
        axios
          .post('/user/existsByEmail', null, { params: { email: userDTO.email } })
          .then(response => {
  ...
  const onSubmit = e => {
    ...
    for (const field of requiredFields) {
      if (!userDTO[field]) {
        setReadCheck(
          `${
            field === 'passwordChk' ? '비밀번호 확인 란 ' : field.toUpperCase()
          } 이 비어있거나 형식에 맞지않습니다.다시 입력해주세요!`
        );
        return; // 회원가입을 중지하고 함수를 빠져나감
      }
    }
    ...
    axios
      .post('/user/accountWrite', null, { params: userDTO })
      .then(res => {
        alert('회원가입을 축하합니다.');
  ...
  return (
    <>
      <div className="WriteBox">
        ...
        <p>&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp; 또는</p>
        ...
        <input
          ...
          onBlur={onFocusEmail}
          placeholder="이메일( @를 포함한 이메일 형식으로 입력하세요 )"
        />
        <br />
        ...
        <div className="readCheck">
          {readCheck} <br /> {emailValidationError}
        </div>
      </div>
