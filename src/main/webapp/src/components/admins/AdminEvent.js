...
const AdminEvent = () => {
  ...
  useEffect(() => {
    axios
      .get(
        `/admin/getEventList?page=${page}&size=${size}&sort=${sort},${sortDirection}&searchKey=${searchKey}`
      )
      .then((res) => {
  ...
  return (
    <>
      <AdminHeader />
      <div
        className="container py-3 rounded bg-secondary-subtle"
      ...
      >
        <div className="row h-100">
          ...
          <div className="col col-12 col-xs-12 col-sm-12 col-md-12 col-lg-4 col-xl-4 my-3">
            <EventAdd couponList={couponList} />
          </div>
        </div>
      </div>
    </>
