...
const ShortURL = () => {
  ...
  const shareShort = () => {
    ...
    axios
      .post('https://naveropenapi.apigw.ntruss.com/util/v1/shorturl', null, {
        params: {
      ...
      .then(res => setResultUrl(res.data.result.url))
      .catch(error => console.log(error));
  };

  return (
    <div>
      <CopyToClipboard text={resultUrl} onCopy={() => alert('클립보드에 URL이 복사되었습니다.')}>
        <img src={image} onLoad={shareShort} style={{ cursor: 'pointer' }} />
      </CopyToClipboard>
    </div>
  );
};
